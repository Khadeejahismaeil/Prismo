...
import { contrastRatio, toHex, colorDist, type RGB } from "./wcag";

// Re-exported for back-compat with existing server-side callers.
export { contrastRatio, toHex, type RGB };
...
/** fg/bg colours in a box, plus `bgShare` = the dominant background colour's
 *  fraction of sampled pixels. A low share means a non-uniform region (gradient
 *  or image behind the text) where a single contrast number would be a guess. */
export function fgBg(raw: Buffer, W: number, ch: number, box: Box): { fg: RGB; bg: RGB; bgShare: number } {
  ...
  const bgShare = (sorted[0]?.c ?? 0) / total;
  // foreground = the colour most different from bg with a meaningful share (the ink)
  let fg = bg;
  let best = -1;
  for (const e of sorted) {
    if (e.c / total < 0.01) continue;
    const d = colorDist(e.rgb, bg);
    if (d > best) {
      best = d;
      fg = e.rgb;
    }
  }
  return { fg, bg, bgShare };
}

/** Image dimensions without decoding all pixels. */
export async function imageSize(buffer: Buffer): Promise<{ W: number; H: number }> {
  const m = await sharp(buffer).metadata();
  return { W: m.width ?? 0, H: m.height ?? 0 };
}

/** Decode an image to raw RGBA pixels + dimensions (for pixel sampling). */
export async function rawPixels(buffer: Buffer): Promise<{ raw: Buffer; W: number; H: number; ch: number }> {
  const { data, info } = await sharp(buffer).ensureAlpha().raw().toBuffer({ resolveWithObject: true });
  return { raw: data as Buffer, W: info.width, H: info.height, ch: info.channels };
}

export async function imagePalette(buffer: Buffer): Promise<{ hex: string; pct: number }[]> {
...
export async function measureDesign(buffer: Buffer): Promise<Measurements> {
  ...
  return {
    ...
    height: H,
    elements,
    palette: await imagePalette(buffer),
